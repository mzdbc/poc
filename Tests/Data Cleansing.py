...
#host가 들어간 컬럼만 뽑아서 csv_table이라는 df 생성
csv_table = raw_df.select([col(c) for c in raw_df.columns if "host" in c])
display(csv_table)

#host가 들어가지 않은 컬럼만 뽑아서 json_table이라는 df 생성
json_table = raw_df.select([col("host_id")]+[col(c) for c in raw_df.columns if "host" not in c])
display(json_table)
...
# MAGIC %fs ls /mnt/dbacademy-users/jungminkim@mz.co.kr/scalable-machine-learning-with-apache-spark/split

# COMMAND ----------

#df를 json 파일로 저장
json_table.write.format('json').mode('append').save('/mnt/dbacademy-users/jungminkim@mz.co.kr/scalable-machine-learning-with-apache-spark/split')

json_df = spark.read.format("json").load('/mnt/dbacademy-users/jungminkim@mz.co.kr/scalable-machine-learning-with-apache-spark/split')
...
#제대로 분할되었는지 확인하기 위해 column 개수 출력
raw_cnum = len(raw_df.columns)
csv_cnum = len(csv_table.columns)
json_cnum = len(json_df.columns)
print("Num of raw_df columns: ",raw_cnum)
print("Num of csv_table columns: ",csv_cnum)
print("Num of json_df columns: ",json_cnum)

# COMMAND ----------

#두 df join
joined_df = csv_table.join(json_df,on="host_id",how="inner")
joined_df.drop_duplicates()
display(joined_df)
joined_cnum = len(joined_df.columns)
print("Num of joined_df columns: ",joined_cnum)
